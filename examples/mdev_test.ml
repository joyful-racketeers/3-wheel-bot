...
let no_arg_command summary f =
  Command.basic ~summary (Command.Param.return f)

let forward =
  no_arg_command "Move the robot forward"
    (fun () ->
...
let buzzer =
  no_arg_command "Make some noise!"
    (fun () ->
...
let servo =
  no_arg_command "Exercise the sonic servo"
    (fun () ->
       for d = 0 to 100 do
         Servo.set_direction Servo.sonar (float_of_int d /. 100.);
         ignore (Unix.nanosleep 0.01 : float)
